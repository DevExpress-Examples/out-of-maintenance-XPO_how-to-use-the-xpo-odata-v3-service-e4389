    ...
    Partial Public Class ClientForm
        ...
        Private Sub gvlODataV3Service_FocusedRowChanged(ByVal sender As Object, ByVal e As FocusedRowChangedEventArgs) Handles gvlODataV3Service.FocusedRowChanged
            ...
            If value Is Nothing Then
                pictureBox.Image = Nothing
            Else
                Dim categoryID As Integer = DirectCast(value, Integer)
        ...
        Private Sub entityToShow_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles entityToShow.SelectedIndexChanged
            ...
            Try
                ...
                ds.AddRange(DirectCast(queue.Execute(), IEnumerable(Of Object)))
                ListObjects = ds
        ...
        Private Sub GetAnnotation()
            ...
            Dim annotatedModel As IEdmModel = Nothing
            Dim errors As IEnumerable(Of EdmError) = Nothing
            Dim xmlReader As XmlReader = System.Xml.XmlReader.Create(New StringReader(metadata))
            ...
            For Each annotation As IEdmVocabularyAnnotation In annotatedModel.VocabularyAnnotations
                If annotation.Term.TermKind <> EdmTermKind.Value Then
                    Continue For
                End If
                Dim valueAnnotation As IEdmValueAnnotation = DirectCast(annotation, IEdmValueAnnotation)
                Dim [property] As IEdmProperty = DirectCast(valueAnnotation.Target, IEdmProperty)
                Dim term As IEdmTerm = valueAnnotation.Term
                Dim entityTypeName As String = DirectCast([property].DeclaringType, IEdmSchemaType).Name
                ...
                Select Case term.Name
                    Case "Size"
                        annotations(entityTypeName)([property].Name).Size = CInt((DirectCast(valueAnnotation.Value, IEdmIntegerValue).Value))
                    Case "ReadOnly"
                        annotations(entityTypeName)([property].Name).ReadOnly = DirectCast(valueAnnotation.Value, IEdmBooleanValue).Value
                End Select
            Next annotation
        End Sub

        Private Sub entity_PropertyChanged(ByVal sender As Object, ByVal e As PropertyChangedEventArgs)
            Dim propertiesAnnotattion As Dictionary(Of String, XpoAnnotation) = Nothing
            If Not annotations.TryGetValue(sender.GetType().Name, propertiesAnnotattion) Then
                Return
            End If
            Dim annotation As XpoAnnotation = Nothing
            ...
            If annotation.Size > 0 Then
                If value.GetType() Is GetType(Byte()) AndAlso DirectCast(value, Byte()).Length > annotation.Size Then
                    Throw New ArgumentException(e.PropertyName)
                End If
                If value.GetType() Is GetType(String) AndAlso DirectCast(value, String).Length > annotation.Size Then
                    Throw New ArgumentException(e.PropertyName)
                End If
            End If
        End Sub
        ...
        Public Function GetProperyValue(ByVal entity As Object, ByVal propertyName As String) As Object
            ...
            Dim properyGetters As Dictionary(Of String, Func(Of Object, Object)) = Nothing
            ...
            End If
            Dim func As Func(Of Object, Object) = Nothing
        ...
        Private Sub checkRemove_Click(ByVal sender As Object, ByVal e As EventArgs) Handles checkRemove.Click
            ...
            If view.Columns.Count < 1 OrElse view.FocusedRowHandle < 0 Then
                Return
            End If
            Dim productId As Integer = CInt((view.GetRowCellValue(view.FocusedRowHandle, view.Columns("ProductID"))))
